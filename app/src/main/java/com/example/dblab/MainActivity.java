...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Cursor c = con.query(DbHelper.TABLE_NAME, null, null, null, null, null, db.KEY_ID, null);
        List<String> dogs = new ArrayList<>();
        if(c.moveToFirst()) {
            int nameIdx = c.getColumnIndex(db.KEY_NAME);
            int idIdx = c.getColumnIndex(db.KEY_ID);
            int ageIdx = c.getColumnIndex(db.KEY_AGE);
            while (c.moveToNext()) {
                dogs.add(String.format("(%d)      %s  Age - (%d)", c.getInt(idIdx), c.getString(nameIdx), c.getInt(ageIdx))) ;
            }
